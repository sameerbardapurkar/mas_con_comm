    ...
    for(int j=0;j<total_robots; j++)
      {
        ...
        {
          ROS_INFO("Communication Epsilon is %f, for robot %d,%d, eval cost is %f, original cost is %f",communicationEpsilon, j,i, eval_cost, robots_[j].getOriginalCost());
          communicationRequired = true;
        }
      }
